...
class FakeNative2qMontreal(FakeBackend):
    """A fake 27 qubit backend."""

    def __init__(self):
        dirname = os.path.dirname(__file__)
        filename = "conf_native_2q_montreal.json"
        with open(os.path.join(dirname, filename)) as f_conf:
            conf = json.load(f_conf)

        configuration = PulseBackendConfiguration.from_dict(conf)
        configuration.backend_name = 'fake_native_2q_montreal'
        self._defaults = None
    ...
    def properties(self):
        ...
        if not self._properties:
            dirname = os.path.dirname(__file__)
            filename = "props_native_2q_montreal.json"
    ...
    def defaults(self):
        ...
        if not self._defaults:
            dirname = os.path.dirname(__file__)
            filename = "defs_native_2q_montreal.json"
